...
def _generated_result(run_dir: Path, quiz_mode: str) -> dict[str, Any] | None:
    ...
    return {
        ...
        "regeneration_attempts_used": int(manifest.get("regeneration_attempts_used") or 0),
        "max_regeneration_attempts": int(manifest.get("max_regeneration_attempts") or 1),
...
def question_review(run_id: str, body: QuestionReviewBody):
    ...
    # Re-read from disk AFTER Notebook 06 has finished. _quiz_manifest now
    # chooses the freshest manifest, so a newly regenerated base manifest cannot
    # be hidden by an older MCP-visual copy.
    payload = _assessment_status_payload(str(run_id))
        ...
        if not changed:
            raise HTTPException(
                ...
                detail=(
                    ...
                    "regenerated. Try again with a more specific reason."
                ),
            )
    ...
    payload["question_review_result"] = {
        ...
        "regeneration_commit_gate": (
            commit_gate
            if (
                body.action == "regenerate"
                and "commit_gate" in locals()
            )
            else None
        ),
    }
