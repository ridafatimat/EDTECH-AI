...
```mermaid
flowchart TD
    A[Lesson transcript] --> B[Agent 1]
    B --> C[Preprocessing & cleaning]
    C --> D[Technical terminology normalisation]
    D --> E[Semantic chunking]
    E --> F[Topic extraction]
    F --> G[AQA syllabus mapping]
    G --> H[Human review / HITL]
    H --> I[Approved topics]
    I --> J[Agent 2]
    J --> K[Official question retrieval]
    K --> L[Coverage / mark shortfall detection]
    L --> M[AI generation when required]
    M --> N[Visual rendering]
    N --> O[Question / quiz review]
    O --> P[Final question paper + marking scheme PDF]
```
...
```mermaid
flowchart TD
    FE[edtech_frontend/ — Next.js / pnpm] --> API[api/ — FastAPI]
    API --> SI[streamlit_integration/ — current runtime bridge]
    SI --> LG[langgraph_orchestration/ — LangGraph workflow + PostgreSQL checkpoints]
    LG --> MCP[mcp_server/]
    MCP --> A1[Agent_1/ — transcript processing, topic extraction, AQA syllabus mapping]
    MCP --> A2[Agent2/ — question retrieval, quiz generation, visual rendering, final PDF]
```
...
```mermaid
flowchart TD
    A[Transcript] --> B[Preprocessing]
    B --> C[Technical normalisation]
    C --> D[Semantic chunking]
    D --> E[Topic candidate extraction]
    E --> F[Evidence-quality evaluation]
    F --> G[AQA syllabus mapping]
    G --> H[Human review]
    H --> I[Approved topics]
```
...
```mermaid
flowchart TD
    A[Approved topics] --> B[Official question retrieval]
    A --> C[Complete AI quiz generation]
```
...
```mermaid
flowchart TD
    A[Approved topics] --> B[Qdrant question retrieval]
    B --> C[Topic / paper filtering]
    C --> D[Re-ranking]
    D --> E[Retrieval HITL feedback]
    E --> F[Approved retrieved questions]
```
...
```mermaid
flowchart TD
    A[Official retrieved questions] --> C[Final hybrid assessment]
    B[AI-generated missing coverage] --> C
```
...
```mermaid
flowchart TD
    A[Quiz plan] --> B[Single consolidated generation call]
    B --> C[Questions + marking guidance]
```
...
```mermaid
flowchart TD
    A[Try consolidated generation] --> B[Validate output]
    B --> C{Valid?}
    C -->|Yes| D[Accept]
    C -->|No| E[Targeted fallback]
```
...
1. Separate runtime and diagnostic logic from UI-specific Streamlit code
2. Move required runtime components into neutral backend / orchestration locations
3. Update imports and paths
4. Verify the project from a fresh clone
5. Then remove the remaining obsolete Streamlit UI components
